...
GAIA_TABLE = "/Users/csayres/fpscommis/gaia/gaiaFieldsAll.txt"
...
def grabGaiaCoords(raCen, decCen):
    ...
    tstart = time.time()
    gaia = pandas.read_csv(GAIA_TABLE, sep="|", skipinitialspace=True, names=names, dtype=dtypes)
...
def processBundle(imgBaseDir, imgNumber, plot=True):
    #########################
    ...
    for gfaid in gfaList:
        ...
        filename = imgBaseDir + "gimg-gfa%in-%s.fits.gz"%(gfaid, imgNumStr)
    ...
    for gfaID, imgData in imgBundle.items():
        ...
        keep = (xCCD > 0) & (xCCD < 2048) & (yCCD >0) & (yCCD < 2048)
        xCCD = xCCD[keep]
        yCCD = yCCD[keep]
        _xWokGaia = xWok[keep]
        _yWokGaia = yWok[keep]
...
if __name__ == "__main__":
    imgDir = "/Volumes/futa/apo/data/gcam/59562/"
    print("on axis\n-------")
    processBundle(imgDir, 194)  #3:06 UTC (guiding on axis)
    plt.show()

    # print("\n\ncherno offset 0 0 +0.1\n-------")
    # processBundle(imgDir, 930)  #(guiding on axis at cherno offset 0 0 -0.1)
    # # plt.show()

    # print("\n\ncherno offset 0 0 -0.1\n-------")
    # processBundle(imgDir, 965)  #(guiding on axis at cherno offset 0 0 -0.1)
    # plt.show()
